...
- `u` reviews missing commits and updates the selected repository after confirmation. `Esc` from the review returns to the loaded list.
...
When a global repository has commits on its origin that are not in the local checkout, `/list` checks for them in the background and shows a green one-line repository-update notice from the `CLAUDE` column. Failed checks stay out of the interface. `u` opens the missing commit subjects only when the selected repository has an update. Entering `y` runs a fast-forward-only `git pull` and refreshes the list; `Esc` returns to the loaded list without pulling.
